...
function nodeFromBackend(raw: Record<string, unknown>): AnyNodeData {
  ...
  return {
    ...
    ...(raw.bubble_style !== undefined && { bubbleStyle: raw.bubble_style as string }),
    ...(raw.bubble_target_id !== undefined && { bubbleTargetId: raw.bubble_target_id as string }),
    ...(raw.speaker_name !== undefined && { speakerName: raw.speaker_name as string }),
  ...
  toBackendDict(): Record<string, unknown> {
    ...
    for (const [id, node] of Object.entries(this.data.nodes)) {
      nodes[id] = {
        ...
        ...(node as any).content !== undefined && { content: (node as any).content },
        ...(node as any).bubbleStyle !== undefined && { bubble_style: (node as any).bubbleStyle },
        ...(node as any).bubbleTargetId !== undefined && { bubble_target_id: (node as any).bubbleTargetId },
        ...(node as any).speakerName !== undefined && { speaker_name: (node as any).speakerName },
        ...(node as any).fontFamily !== undefined && { font_family: (node as any).fontFamily },
        ...(node as any).fontSize !== undefined && { font_size: (node as any).fontSize },
        ...(node as any).color !== undefined && { color: (node as any).color },
        ...(node as any).textAlign !== undefined && { text_align: (node as any).textAlign },
        ...(node as any).characterId !== undefined && {
          character_id: (node as any).characterId,
          active_layers: (node as any).activeLayers,
          available_layers: (node as any).availableLayers,
          frame_sequence: (node as any).frameSequence,
        },
        ...(node as any).assetPath !== undefined && { asset_path: (node as any).assetPath },
        ...(node as any).parallaxSpeed !== undefined && { parallax_speed: (node as any).parallaxSpeed },
        ...(node as any).blur !== undefined && { blur: (node as any).blur },
        ...(node as any).zoom !== undefined && { zoom: (node as any).zoom },
        ...(node as any).targetNodeId !== undefined && { target_node_id: (node as any).targetNodeId },
        ...(node as any).volume !== undefined && { volume: (node as any).volume },
        ...(node as any).loop !== undefined && { loop: (node as any).loop },
        ...(node as any).startTime !== undefined && { start_time: (node as any).startTime },
        ...(node as any).duration !== undefined && { duration: (node as any).duration },
        ...(node as any).audioType !== undefined && { audio_type: (node as any).audioType },
        ...(node as any).interactive !== undefined && { interactive: (node as any).interactive },
      };
    }
