 ...
 *
 * Two modes:
 * 1. Script mode: paste lines with character + emotion hints
 * 2. SRT mode: paste SRT subtitle file
 *
 * After importing, calls the automation API to build a SceneGraph
...
export const ScriptImport: React.FC<{ onClose?: () => void }> = ({ onClose }) => {
    ...
    useEffect(() => {
        if (mode === 'script') {
            ...
            // For SRT, extract character names from "Name: text" format
            const lines = srtText.split('\n').filter(l => {
                const m = l.match(/^([^:：]{1,20})[:\s：]\s*.+$/);
                return m && !l.match(/^\d/) && !l.includes('-->');
            });
    ...
    return (
        <div className="flex flex-col h-full text-white text-xs">
            ...
            <div className="flex-1 min-h-0 p-2 overflow-y-auto space-y-2">
                <textarea
                    value={mode === 'script' ? scriptText : srtText}
                    onChange={e => mode === 'script' ? setScriptText(e.target.value) : setSrtText(e.target.value)}
                    placeholder={mode === 'script'
                        ? 'Hoa: Chào bạn!\nNam (vui): Chào!'
                        : '1\n00:00:00,000 --> 00:00:02,500\nHoa: Chào bạn!'
                    }
                    className="w-full h-32 bg-black/40 rounded-lg p-2.5 text-[11px] font-mono text-neutral-300 border border-white/10 focus:border-emerald-500/50 focus:outline-none resize-none"
                    spellCheck={false}
                />
