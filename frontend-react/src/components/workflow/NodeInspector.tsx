...
import { useWorkflowStore, type CharacterNodeData, type CharacterV2NodeData, type BackgroundNodeData, type ForegroundNodeData, type StageNodeData, type StageLayer, type CameraNodeData, type CameraKeyframe, type SceneNodeData, type MapNodeData } from '@/stores/useWorkflowStore';
import { useAppStore, STATIC_BASE } from '@/stores/useAppStore';
import { useCharacterV2Store } from '@/stores/useCharacterV2Store';
import { API_BASE_URL } from '@/config/api';
import { X, User, Image, Film, Trash2, Upload, Check, MapPin, Sparkles, Layers, Clapperboard, Plus, Eye, EyeOff, ChevronDown, ChevronRight, ArrowUp, ArrowDown, Maximize2, Video } from 'lucide-react';
        ...
        <div className="flex flex-col h-full">
            ...
            <div className="px-4 py-3 border-b border-white/5 flex items-center justify-between">
                <div className="flex items-center gap-2">
                    ...
                    {nodeType === 'foreground' && <Layers className="w-4 h-4 text-cyan-400" />}
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
                ...
                {nodeType === 'foreground' && (
                    <>
                        <ForegroundPicker
                            selectedPath={(data as ForegroundNodeData).assetPath || ''}
                            onSelect={(path) => updateNodeData(selectedNode.id, { assetPath: path })}
                        />

                        {/* Foreground Preview */}
                        {(data as ForegroundNodeData).assetPath && (
                            <div className="w-full aspect-video bg-black/30 rounded-lg overflow-hidden border border-white/5">
                                <img
                                    src={`${STATIC_BASE}/${(data as ForegroundNodeData).assetPath}`}
                                    className="w-full h-full object-cover"
                        ...
                        <FieldGroup label="Position X (units)">
                            <NumberInput
                                value={+((data as ForegroundNodeData).posX / ppu).toFixed(2)}
                                onChange={(v) => updateNodeData(selectedNode.id, { posX: Math.round(v * ppu) })}
                                step={0.1}
                            />
                        </FieldGroup>
                        <FieldGroup label="Position Y (units)">
                            <NumberInput
                                value={+((data as ForegroundNodeData).posY / ppu).toFixed(2)}
                                onChange={(v) => updateNodeData(selectedNode.id, { posY: Math.round(v * ppu) })}
                                step={0.1}
                            />
                        </FieldGroup>
                        <FieldGroup label="Z-Index">
                            <NumberInput
                                value={(data as ForegroundNodeData).zIndex}
                                onChange={(v) => updateNodeData(selectedNode.id, { zIndex: v })}
                                min={0}
                                max={100}
                            />
                        ...
                        <FieldGroup label="Height (units)">
                            <NumberInput
                                value={+((data as ForegroundNodeData).scale / ppu).toFixed(2)}
                                onChange={(v) => updateNodeData(selectedNode.id, { scale: Math.round(v * ppu) })}
                                step={0.1}
                                min={+(100 / ppu).toFixed(2)}
                                max={+(2000 / ppu).toFixed(2)}
                            />
                        </FieldGroup>
                        <FieldGroup label="Opacity">
                            <NumberInput
                                value={(data as ForegroundNodeData).opacity}
                                onChange={(v) => updateNodeData(selectedNode.id, { opacity: v })}
                                step={0.1}
                                min={0}
                                max={1}
                        ...
                        <FieldGroup label="Blur">
                            <NumberInput
                                value={(data as ForegroundNodeData).blur}
                                onChange={(v) => updateNodeData(selectedNode.id, { blur: v })}
                                min={0}
                                max={20}
                            />
            ...
            >
                {uploading ? (
                    ...
                    <div className="flex items-center justify-center gap-2 text-neutral-500">
                        ...
                        <span className="text-[10px]">Drop image/PSD or click to upload</span>
                    </div>
                )}
                <input
                    ref={inputRef}
...
// ── Foreground Library Picker ──
interface FgItem { name: string; path: string; url: string; size: number; }

function ForegroundPicker({ selectedPath, onSelect }: { selectedPath: string; onSelect: (path: string) => void }) {
    const [items, setItems] = useState<FgItem[]>([]);
    const [uploading, setUploading] = useState(false);
    const [dragOver, setDragOver] = useState(false);
    const inputRef = useRef<HTMLInputElement>(null);

    const fetchFgs = useCallback(async () => {
        try {
            const res = await fetch(`${API_BASE_URL}/api/foregrounds`);
            if (res.ok) setItems(await res.json());
        } catch { /* ignore */ }
    }, []);

    useEffect(() => { fetchFgs(); }, [fetchFgs]);
    ...
    const handleUpload = useCallback(async (files: FileList | File[]) => {
        ...
        setUploading(true);
        try {
            const res = await fetch(`${API_BASE_URL}/api/foregrounds/upload`, { method: 'POST', body: formData });
            if (res.ok) {
                const data = await res.json();
                if (data.uploaded?.length > 0) {
                    onSelect(data.uploaded[0].path);
    ...
    return (
        <div className="space-y-2">
            <label className="block text-[10px] font-semibold text-neutral-400 uppercase tracking-wider">Foreground Library</label>

            {/* Upload zone */}
            <div
                className={`w-full border-2 border-dashed rounded-lg p-3 text-center cursor-pointer transition-all ${dragOver ? 'border-cyan-500/50 bg-cyan-500/10' : 'border-white/10 hover:border-white/20'
                    }`}
                onDragOver={(e) => { e.preventDefault(); setDragOver(true); }}
                onDragLeave={() => setDragOver(false)}
                onDrop={handleDrop}
            ...
            {items.length > 0 ? (
                <div className="grid grid-cols-3 gap-1.5 max-h-48 overflow-y-auto scrollbar-thin">
                    {items.map((fg) => (
                        <button
                            key={fg.name}
                            onClick={() => onSelect(fg.path)}
                            className={`aspect-video rounded-lg overflow-hidden border-2 transition-all ${selectedPath === fg.path
                                ? 'border-cyan-500 shadow-lg shadow-cyan-500/30 scale-[1.02]'
                                : 'border-transparent hover:border-white/20'
                                }`}
                        >
                            <img
                                src={`${STATIC_BASE}/${fg.path}`}
                                className="w-full h-full object-cover"
                                alt={fg.name}
                                loading="lazy"
                            />
                            {selectedPath === fg.path && (
                ...
                <p className="text-[10px] text-neutral-600 text-center py-3">No foregrounds yet. Upload one above!</p>
            )}
        </div>
    );
}
